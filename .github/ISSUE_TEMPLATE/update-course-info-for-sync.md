---
name: Update your course's info for syncs/OTTR updates
about: Provide information about your moved/renamed Quarto course (which uses the OTTR Quarto Template) so that it can continue to be enrolled in OTTR updates
title: ''
labels: ''
assignees: |
  kweav
  carriewright11

---

<!-- Remove headings if they are not applicable to the changes/updates you made for your repo -->

## Please provide the new GitHub link to your repository

<!-- The GitHub link for this repository is https://github.com/ottrproject/OTTR_Quarto. Please make sure that the link you provide starts with github.com! -->

## If you can, please provide the old GitHub link before your repository was renamed or moved. 

<!-- This is optional, but would be nice if you could provide it for us --> 
